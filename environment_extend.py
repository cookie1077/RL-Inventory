...
class InventoryEnv_plus(gym.Env):
    def __init__(self, fixed_cost=0, lead_time=0, mean=5, std=1, p=4.0, alpha=1):
        ...
        #demand info incoporation
        self.state = np.append(self.state, self.demand_mean)
        self.state = np.append(self.state, self.demand_std)
        # setting viewer
        self.viewer = None
        ...
        # inventory level, on-order items, mean, std
        ...
        #expanding state space to incorporate mean value
        self.obs_low = np.append(self.obs_low,self.min_inv)
        self.obs_high = np.append(self.obs_high,self.max_inv)

        #expanding state space to incorporate std value
        self.obs_low = np.append(self.obs_low,0)
        self.obs_high = np.append(self.obs_high,self.max_order - self.min_order)
    ...
    def step(self, action, translate=True, evaluate=False, penalty=False):
        ...
        #mean, std information incorporation
